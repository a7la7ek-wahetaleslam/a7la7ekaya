    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(lToggle) {
            lToggle.addEventListener('click', () => {
                ...
                const translations = { ar: { logo: "واحة الإسلام", home: "الرئيسية", forum: "العودة للمنتدى", radio_side: "البث المباشر", radio: "راديو مباشر", search_results: "نتائج البحث" }, en: { logo: "Wahet Eslam", home: "Home", forum: "Back to Forum", radio_side: "Live Broadcast", radio: "Live Radio", search_results: "Search Results" } };
                if(qOverlay) {
                ...
                if (currentActiveSurahTag) {
                    ...
                    if(pSurah) pSurah.textContent = currentActiveSurahTag.getAttribute('data-' + currentLang);
                    if(pReciter) pReciter.textContent = currentActiveSurahTag.getAttribute('data-reciter-' + currentLang);
                }
    ...
    function toggleQuranLibrary() {
        ...
        if (isOpening) {
            ...
            libOverlay.style.display = 'block';
            ...
            if(!audio.src || audio.src === "" || (audio.paused && audio.currentTime === 0)) {
    ...
    function updateToggleButtonText() {
        ...
        if(!toggleBtn) return;
        if(document.body.classList.contains('quran-active')) {
            toggleBtn.innerHTML = '<span class="material-symbols-outlined" style="font-size:18px;">forum</span> ' + (currentLang === 'ar' ? 'العودة للمنتدى' : 'Back to Forum');
        } else {
            toggleBtn.innerHTML = '<span class="material-symbols-outlined" style="font-size:18px;">mosque</span> ' + (currentLang === 'ar' ? 'فتح المكتبة' : 'Open Library');
        }
    }
        ...
        reciters.forEach(rec => {
            ...
            targetSurahs.forEach(surah => {
                ...
                allSearchTracks.push({
                    src: rec.prefix + surah.id + '.mp3', ar: 'سورة ' + surah.ar, en: 'Surah ' + surah.en,
                    reciterAr: rec.ar, reciterEn: rec.en, searchStr: ('سورة ' + surah.ar + ' ' + cleanSurahAr + ' ' + rec.ar + ' surah ' + surah.en + ' ' + rec.en).toLowerCase()
                });
            });
        });
        adhkarAndDuas.forEach(item => { allSearchTracks.push({ src: item.src, ar: item.ar, en: item.en, reciterAr: 'مكتبة واحة الإسلام', reciterEn: 'Wahet Al-Eslam', searchStr: (item.ar + ' ' + item.en + ' مكتبة واحة الإسلام wahet al-eslam').toLowerCase() }); });
    }
    ...
    function openAdhkarView() {
        ...
        if(dynamicSurahsContainer) {
            ...
            adhkarAndDuas.forEach(item => {
                let sCard = document.createElement('a'); sCard.className = "surah-card surah-link"; sCard.setAttribute('data-src', item.src); sCard.setAttribute('data-ar', item.ar); sCard.setAttribute('data-en', item.en); sCard.setAttribute('data-reciter-ar', 'مكتبة واحة الإسلام'); sCard.setAttribute('data-reciter-en', 'Wahet Al-Eslam');
    ...
    function playSurahFromTag(tagElement) {
        ...
        if(audio) {
            ...
            let pSurah = document.getElementById('playerSurah'); let pReciter = document.getElementById('playerReciter');
            if(pSurah) pSurah.textContent = tagElement.getAttribute('data-' + currentLang);
            if(pReciter) pReciter.textContent = tagElement.getAttribute('data-reciter-' + currentLang);
            audio.play();
            var player = document.getElementById('quran-bottom-player');
    ...
    function playLiveRadio() {
        ...
        if(audio) {
            ...
            if(pReciter) pReciter.textContent = currentLang === 'ar' ? "إذاعة القرآن الكريم" : "Quran Radio";
            if(pSurah) pSurah.textContent = currentLang === 'ar' ? "بث مباشر" : "Live Stream";
            audio.play();
            var player = document.getElementById('quran-bottom-player');
            if(player) player.style.display = 'flex';
        }
