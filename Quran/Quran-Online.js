    ...
    function syncActiveDOMTag(src) {
        document.querySelectorAll('.surah-link').forEach(el => el.classList.remove('active'));
        let newActive = document.querySelector(`.surah-link[data-src="${src}"]`);
        if (newActive) {
            currentActiveSurahTag = newActive;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (sessionStorage.getItem('quran_library_isOpen') === 'true') {
            toggleQuranLibrary();
        }
    });
    ...
    function toggleQuranLibrary() {
        ...
        if (isOpening) { 
            ...
            sessionStorage.setItem('quran_library_isOpen', 'true');
        } 
        else { 
            ...
            sessionStorage.setItem('quran_library_isOpen', 'false');
        }
        updateToggleButtonText();
    }
    ...
    function openReciterView(rec) {
        ...
        if(dynamicSurahsContainer) {
            ...
            let savedData = JSON.parse(sessionStorage.getItem(STORAGE_KEY));
            if (savedData && savedData.src) syncActiveDOMTag(savedData.src);
        }
    }
    ...
    function playAudio(src, sAr, sEn, rAr, rEn) {
        ...
        if(audio) {
            ...
            let playPromise = audio.play();
            if (playPromise !== undefined) { playPromise.catch(error => { if(playBtn) playBtn.textContent = 'play_arrow'; }); }
            
            var player = document.getElementById('quran-bottom-player');
            if(player) player.style.display = 'flex';
            
            syncActiveDOMTag(src);
        }
    }
    ...
    function playSurahFromTag(tagElement) {
        ...
        playAudio(src, sAr, sEn, rAr, rEn);
    }

    function playNextSurah() {
        let savedData = JSON.parse(sessionStorage.getItem(STORAGE_KEY));
        if (!savedData || !savedData.src) return;
        let currentSrc = savedData.src;

        let aIdx = adhkarAndDuas.findIndex(item => item.src === currentSrc);
        if (aIdx !== -1) {
            if (aIdx < adhkarAndDuas.length - 1) {
                let next = adhkarAndDuas[aIdx + 1];
                playAudio(next.src, next.ar, next.en, savedData.rAr, savedData.rEn);
            }
            return;
        }

        let reciter = reciters.find(r => currentSrc.startsWith(r.prefix));
        if (reciter) {
            let surahId = currentSrc.replace(reciter.prefix, '').replace('.mp3', '');
            let sIdx = targetSurahs.findIndex(s => s.id === surahId);
            if (sIdx !== -1 && sIdx < targetSurahs.length - 1) {
                let nextSurah = targetSurahs[sIdx + 1];
                let nextSrc = reciter.prefix + nextSurah.id + '.mp3';
                playAudio(nextSrc, 'سورة ' + nextSurah.ar, 'Surah ' + nextSurah.en, reciter.ar, reciter.en);
            }
        }
    }
    ...
    function playPrevSurah() {
        ...
        if (aIdx !== -1) {
            if (aIdx > 0) {
                let prev = adhkarAndDuas[aIdx - 1];
                playAudio(prev.src, prev.ar, prev.en, savedData.rAr, savedData.rEn);
            }
            return;
        }
        ...
        if (reciter) {
            ...
            let sIdx = targetSurahs.findIndex(s => s.id === surahId);
            if (sIdx > 0) {
                let prevSurah = targetSurahs[sIdx - 1];
                let prevSrc = reciter.prefix + prevSurah.id + '.mp3';
                playAudio(prevSrc, 'سورة ' + prevSurah.ar, 'Surah ' + prevSurah.en, reciter.ar, reciter.en);
            }
        }
    }
